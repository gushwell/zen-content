...
本書は、
> MCPサーバー開発から MCPホスト開発、エージェント作成まで、MCPエコシステム全体を一気通貫で構築するための実践書です。
― 書籍「はじめに」より抜粋

という趣旨の書籍であり、MCP を体系的に学べる内容になっています。
第1回となる今回は、第3章に掲載されている 電卓 MCP サーバー（calculator_server.py） を C# へ移植してみます。
