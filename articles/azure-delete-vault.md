...
```
<WorkloadName> UnRegister Completed
...
複数のサブスクリプションがある場合は、サブスクリプションの選択が表示されますので、該当するサブスクリプションを選択します。
...
3. Storage Containerの登録解除スクリプトを実行
4. Storage Containerの `UnRegister Completed` を確認
